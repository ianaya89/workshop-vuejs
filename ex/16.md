...
___
### 🤓 Extras
El *toaster* no necesariamente debe usarse para mostrar errores, podemos usarlo también para mostrar informarle al usuario que la información se obtuvo de manera correcta. Podemos usar el mismo patron para mostrar el *toaster* dentro del método `then()` de la *Promise* con un mensaje como 'Artists retrieved successfully'. Deberíamos también crear una nueva variable en `Search.vue` que nos permita pasar una valor dinámico a la propiedad `type` de `Toaster.vue` y de esta manera cambiar el color del *toaster* segun el tipo de error.
